  ...
  <view class="page">
    ...
    <view class="card">
      ...
      <view v-if="currentQuestion" class="question">
        ...
        <view class="output-requirement-wrap">
          <text class="output-requirement-title">📌 {{ t.aliasReq }}</text>
          <text class="output-requirement-content" v-if="displayQuestion?.required_output_columns || currentQuestion?.required_output_columns">{{ displayQuestion?.required_output_columns ?? currentQuestion?.required_output_columns }}</text>
          <text class="output-requirement-content output-requirement-hint" v-else>{{ t.outputReqHint }}</text>
        </view>
...
const UI_STRINGS = [
  {
    ...
    skip: "跳过",
    submit: "提交",
    outputReqHint: "系统正在解析标准答案列名，请稍后刷新；或参考题目描述中的列名要求，在 SELECT 中用 AS 起对应别名。",
  ...
  },
  {
    ...
    skip: "Skip",
    submit: "Submit",
    outputReqHint: "Parsing required columns… Refresh or use AS in SELECT per the question.",
  ...
  },
  {
    ...
    skip: "跳過",
    submit: "提交",
    outputReqHint: "系統正在解析標準答案列名，請稍後刷新；或參考題目描述中的列名要求，在 SELECT 中用 AS 起對應別名。",
...
const suggestedTimeText = computed(() => {
  const q = displayQuestion.value;
  if (!q) return "";
  const sec = q.suggested_time_seconds ?? q.time_limit_seconds;
...
/** 限时挑战可用秒数（优先教师设置，否则用建议值） */
const challengeTimeSeconds = computed(() => {
  const q = displayQuestion.value;
  if (!q) return 0;
  const sec = q.time_limit_seconds ?? q.suggested_time_seconds;
...
const bootstrap = async () => {
  ...
  questions.value = await getQuestions({ skip: 0, limit: 50 });
...
.output-requirement-content.output-requirement-hint {
  color: #4b5563;
  font-size: 12px;
}
.q-meta-row {
