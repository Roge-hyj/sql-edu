<template>
  <view class="page">
    ...
    <view class="card">
      ...
      <input class="input" v-model="form.title" :placeholder="t.placeholderTitle" />
      <textarea class="textarea" v-model="form.content" :placeholder="t.placeholderContent" auto-height />
      <textarea
        class="textarea"
...
const UI_STRINGS = [
  {
    ...
    formHint: "难度、限时、结果列名由系统根据题目与标准答案自动推断，无需填写。",
    createQuestion: "创建题目",
    saveChanges: "保存修改",
  ...
  },
  {
    ...
    formHint: "Difficulty, time limit and output columns are auto-inferred from the question and correct SQL.",
  ...
  },
  {
    ...
    formHint: "難度、限時、結果列名由系統根據題目與標準答案自動推斷，無需填寫。",
    createQuestion: "創建題目",
    saveChanges: "保存修改",
...
const knowledgePointLabels = computed(() =>
  knowledgePoints.value.map((kp) => `${pickKpText(kp, "level")} · ${pickKpText(kp, "name")}`)
);
...
const form = reactive<{
  ...
  required_output_columns: string;
}>({
  id: 0,
  title: "",
  content: "",
...
const loadData = async () => {
  ...
  // 加载题目列表
  try {
    // 后端限制 limit 最大为 100，所以使用 100
    const result = await getQuestions({ skip: 0, limit: 100 });
...
const onPickKnowledgePoint = (e: any) => {
  ...
  if (idx >= 0 && idx < knowledgePoints.value.length) {
...
const handleGenerateByAI = async () => {
  ...
  const kp = knowledgePoints.value[selectedKpIndex.value];
  if (!kp) return;
  ...
  try {
    ...
    if (n > 0) {
      questions.value = await getQuestions({ skip: 0, limit: 100 });
...
const selectQuestionByIndex = async (idx: number) => {
  ...
  try {
    ...
    form.required_output_columns = fullQuestion.required_output_columns ?? "";
    // 更新列表中的题目信息
    ...
    form.required_output_columns = (q as any).required_output_columns ?? "";
  }
};
