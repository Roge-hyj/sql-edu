...
class TestSQLSafetyCheck:
    ...
    def test_safe_select(self, judge_service):
        """SELECT 语句应该通过安全检查。"""
        assert judge_service._check_sql_safety("SELECT * FROM users") is True

    def test_dangerous_drop(self, judge_service):
        """DROP 语句应该被拒绝。"""
        assert judge_service._check_sql_safety("DROP TABLE users") is False

    def test_dangerous_delete(self, judge_service):
        """DELETE 语句应该被拒绝。"""
        assert judge_service._check_sql_safety("DELETE FROM users") is False

    def test_dangerous_update(self, judge_service):
        """UPDATE 语句应该被拒绝。"""
        assert judge_service._check_sql_safety("UPDATE users SET name = 'test'") is False
    ...
    def test_word_boundary(self, judge_service):
        ...
        assert judge_service._check_sql_safety("SELECT deleted FROM users") is True
