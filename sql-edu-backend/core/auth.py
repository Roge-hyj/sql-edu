...
class AuthHandler(metaclass=SingletonMeta):
    ...
    def _encode_token(self, user_id: int, type: TokenTypeEnum):
        ...
        payload = {
            "iss": user_id,           # iss (Issuer): 我们把用户 ID 藏在这里
            "sub": int(type.value)    # sub (Subject): 标记这个 Token 是类型 1 还是 2
        }
