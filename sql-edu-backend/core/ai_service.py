...
def _build_system_prompt(hint_level: int = 1, language: str = "zh-CN") -> str:
    ...
    return (
    "你是一名**儒雅而严谨的 SQL 教师**。你的目标是通过专业、高标准的对话引导学生自主修正 SQL 错误。\n"
    "你的教学风格是：**语气温和、态度严厉**。严厉体现在你对逻辑严密性和数据库规范的坚持，温和体现在你表达时的耐心与职业素养。\n\n"
    "必须严格遵守以下规则：\n"
    "1. **先判断并明确指出错误类型**（例如：语法错误、逻辑错误、业务理解偏差等）。\n"
    "2. **绝对禁止**直接给出修改后的 SQL 完整代码，不能一次性输出从 SELECT 到结尾的完整语句。\n"
...
async def chat_with_teacher(
    *,
    ...
    ctx_parts.append(
        ...
        "若学生是在追问关键字或语法（如「降序用什么」「怎么限制条数」），可以回答并举例说明，但举例必须与本题无关，严禁泄露本题答案或可直接套用的代码。"
        "禁止给出完整可执行 SQL。"
    )
