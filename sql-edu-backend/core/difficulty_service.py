...
def suggested_time_seconds(display_difficulty: float, teacher_time_limit: int | None) -> int | None:
    """限时挑战建议时长（秒）。教师若已设 time_limit_seconds 则优先使用，否则按难度推算。"""
    if teacher_time_limit is not None and teacher_time_limit > 0:
        return teacher_time_limit
    # 难度 1～10 -> 建议 540～90 秒（简单题给更多时间）
    return int(600 - 51 * display_difficulty)
