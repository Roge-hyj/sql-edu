...
async def _enrich_question_out(
    ...
    if not (required_cols and str(required_cols).strip()):
        required_cols = infer_output_columns_from_sql(question.correct_sql or "")
...
async def get_questions(
    ...
    limit: Annotated[int, Query(ge=1, le=100, description="限制数量")] = 100,
    ...
    for q in questions:
        ...
        if not (required_cols and str(required_cols).strip()):
            required_cols = infer_output_columns_from_sql(q.correct_sql or "")
        out.append(
            QuestionOut(
                id=q.id,
                title=q.title,
...
async def generate_questions_by_ai(
    ...
    for item in items:
        q = Question(
            ...
            required_output_columns=item.get("required_output_columns"),
        )
        session.add(q)
        await session.flush()
...
async def create_question(
    ...
    required_cols = question_data.required_output_columns
    if not (required_cols and str(required_cols).strip()):
        required_cols = infer_output_columns_from_sql(question_data.correct_sql)
    try:
        question = Question(
