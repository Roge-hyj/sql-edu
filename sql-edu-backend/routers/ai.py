...
async def check_sql(
    ...
    try:
        is_correct, error_message = await judge_service.judge_sql(
            payload.student_sql, question.correct_sql
        )
    except SQLJudgeError as e:
        # SQL 执行失败（如语法错误、安全限制等）
        error_message = str(e)
        is_correct = False
    ...
    try:
        ...
        # AI 服务故障：不写入提交记录、不写入对话，本次不计入尝试次数
        raise HTTPException(
    ...
    await chat_repo.add_message(
        ...
        question_id=payload.question_id,
        role="system",
        content=f"【新一轮提交】结果：{'正确' if is_correct else '不正确'}",
    )
    await chat_repo.add_message(
        user_id=user_id,
        question_id=payload.question_id,
    ...
    return SQLCheckResponse(
        is_correct=is_correct,
        hint=ai_hint_result.model_dump(),  # 转换为字典
